...
def find_min_marked(marked_rows, excluded_cols):
    """
...
if __name__ == '__main__':
    ...
    while excluded_cols.__len__() < NODES_NUM:
        ...
        chosen_node, minimum_edge_index = find_min_marked(included_rows, excluded_cols)
        # выделение новой строки
        included_rows[chosen_node] = matr[minimum_edge_index]
        # вычеркивание столбца
        excluded_cols.append(minimum_edge_index)
        # симметричная вставка ребра
        result[chosen_node][minimum_edge_index] = marked_row[minimum_edge_index]
        result[minimum_edge_index][chosen_node] = marked_row[minimum_edge_index]
